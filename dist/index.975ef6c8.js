...
var _countryCard = require("./js/country-card");
const DEBOUNCE_DELAY = 300;
const input = document.querySelector("#search-box");
let div = document.querySelector(".country-info");
let ul = document.querySelector(".country-list");
input.addEventListener("input", (0, _lodashDebounceDefault.default)(onEventInput, DEBOUNCE_DELAY));
function onEventInput(e) {
    ...
    (0, _fetchCountries.fetchCountries)(country).then((data)=>{
        console.log(data);
        if (data.length > 10) (0, _notiflixNotifyAio.Notify).success("Too many matches found. Please enter a more specific name.");
        if (data.length <= 10) {
            console.log(data);
            const listMarkup = data.map((el)=>{
                return (0, _countryCard.countryCardItem)(el);
            });
        ...
        if (data.length === 1) {
            ...
            data.map((el)=>{
                console.log(el);
                div.innerHTML = (0, _countryCard.countryCardList)(el);
                ul.innerHTML = "";
            });
        }
    }).catch((error)=>{
        (0, _notiflixNotifyAio.Notify).success("Oops, there is no country with that name");
        div.innerHTML = "";
        ul.innerHTML = "";
        return error;
    });
...
},{"./css/styles.css":"1CY4s","./js/fetchCountries":"dKuy9","notiflix/build/notiflix-notify-aio":"eXQLZ","lodash.debounce":"3JP5n","./js/country-card":"fBmkr","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"1CY4s":[function() {},{}],"dKuy9":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
},{}],"fBmkr":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "countryCardItem", ()=>countryCardItem);
parcelHelpers.export(exports, "countryCardList", ()=>countryCardList);
const countryCardItem = (cardInfo)=>{
    return `
...
const countryCardList = (listInfo)=>{
    return `
      <div class="country-info__wrapper">
        <img class="country-info__flag" src="${listInfo.flags.svg}" alt="${listInfo.name.official}" width="40" />
        <h2 class="country-info__name">${listInfo.name.official}</h2>
      </div>
      <p class="country-info__capital"><span class="country-info__bold">Capital:</span> ${listInfo.capital}</p>
      <p class="country-info__population"><span class="country-info__bold">Population:</span> ${listInfo.population}</p>
      <p class="country-info__language"><span class="country-info__bold">Languages:</span> ${Object.values(listInfo.languages)}</p>
  `;
};
